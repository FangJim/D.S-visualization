          ...
          .then((result) => {
            if (result) {
              ...
              async function findEmail() {
                await User.findOne({ email: email }, function (err, users) {
                  if (err) throw err;
...
router.post("/exam/searchExam", urlencodedParser, async (req, res) => {
  ...
  let grade = 0;
  console.log(userAns);
  Answer.findOne({ _id: "61da7a1d4fcea08127f4a485" }, function (err, ans) {
    if (err) throw err;
    //console.log(ans.Ans);
    testAns = ans.Ans;
    for (let i = 1; i <= userAns.length; i++) {
      if (testAns[i - 1] === userAns[i - 1]) {
        grade += 20;
      } else {
        errorAns += i.toString();
        errorAns += ",";
