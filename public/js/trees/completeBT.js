...
function drawNode(nodeNumber, level) {
  ...
  } else if (nodeNumber === completeBT_data.length - 1) {
    //the last one
  ...
  if (nodeNumber === 0) {
    ...
    recordMiddle =
      middle[Math.floor(nodeNumber / 2) - 1] + nodeSpacing(nodeNumber);
...
function drawLine(middle) {
  ...
  for (let i = 1; i < middle.length; i++) {
    ...
    if (changeWhite) {
      lChild_color = "white";
      rChild_color = "white";
    } else if (have_rChild && i === middle.length - 2) {
      // have rChild the last one draw red
      lChild_color = "white";
      rChild_color = `rgba(255,0,0,${effect.alpha})`;
    } else if (i === middle.length - 1) {
      // only lChild the last one draw red
...
function drawData() {
  ...
  for (let i = 0; i < completeBT_data.length; i++) {
    ...
    if (changeWhite) {
      ctx.fillStyle = "white";
    } else if (i === completeBT_data.length - 1) {
      //the last one
...
function OrderAnime() {
  if (preOrder_anime || inOrder_anime || postOrder_anime) {
    if (traversalIndex < traversalArr.length) {
      ...
      ctx.beginPath();
      ctx.arc(
        traversalArr[traversalIndex][0],
        traversalArr[traversalIndex][1],
        30,
        0,
        Math.PI * 2
      );
...
insertGo.addEventListener("click", () => {
  ...
  if ((completeBT_data.length - 1) % 2 === 0) {
    //have rChild
    have_rChild = true;
  } else {
    have_rChild = false;
...
tip.addEventListener("click", () => {
  Swal.fire({
    title: "Tips",
    html:
      "每次新增節點到Complete Binary Tree中都必須由上而下,由左至右" +
