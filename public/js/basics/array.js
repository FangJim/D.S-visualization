...
function update() {
  ...
  //塞入值->draw circle 刪除值->draw flash
  flag.state ? drawCircle(flag.goto) : drawSlash(flag.goto);
...
function drawSlash(index) {
  ...
  if (flag.goto != null) {
    ...
    ctx.lineTo(line_x + 100, line_y + 100);
    ctx.strokeStyle = `rgba(255, 0, 0, ${effect.alpha})`;
    ctx.stroke();
  }
}
...
tip.addEventListener("click", () => {
  Swal.fire({
    title: "Tips",
    html:
      "Array是一個連續記憶體配置的資料結構,可以透過索引值去儲存數值,亦可刪除 <br><br>" +
