  ...
  insert(position, element) {
    ...
    if (position > -1 && position < this.length) {
      ...
      if (position === 0) {
        ...
        // after while previous will be in position
        // current will be in position+1
...
function drawLinkedList() {
  ...
  for (let i = 0; i < linkedList.size(); i++) {
    if (effect.alpha < 1) {
      effect.alpha = effect.alpha + 0.02;
    }
...
function buildLinkedList(index, content) {
  ...
  //----------------------build LinkedList------------------------
  //effect
  //new one must be red, indexNow is insert value
  if (index == indexNow) {
    effect.r = 255;
    effect.g = 0;
    effect.b = 0;
...
tip.addEventListener("click", () => {
  Swal.fire({
    title: "Tips",
    html:
      "LinkedList是以串列連接的資料結構,優勢是可以快速插入資料和刪除,但劣勢是搜尋數值必須每次都從頭開始<br><br>" +
